...
try:
    ...
    st.error(f"Lỗi: Thiếu thông tin trong st.secrets: {e}. Vui lòng kiểm tra file secrets.toml (local) hoặc cài đặt secrets trên Streamlit Cloud.")
    st.stop()

try:
    API_KEY = st.secrets["openrouter"]["api_key"]
except KeyError as e:
    st.error(f"Lỗi: Thiếu API key của OpenRouter trong st.secrets: {e}.")
    st.stop()

# API URL cho OpenRouter
